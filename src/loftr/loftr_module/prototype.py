...
class DETRBlock(nn.Module):
    ...
    def forward(self, query: torch.Tensor, query_pe: torch.Tensor, feat: torch.Tensor, feat_pe: torch.Tensor,
        ...
        query_q = self.query_q_proj(query_pe_in).view(bs, -1, self.nhead, self.n_dim)
        query_k = self.query_k_proj(query_pe_in).view(bs, -1, self.nhead, self.n_dim)
        query_v = self.query_v_proj(query).view(bs, -1, self.nhead, self.n_dim)
        message = self.query_self_attention(query_q, query_k, query_v)
        message = message.view(bs, -1, self.nhead * self.n_dim)
        ...
        message_q = self.prototype_q_proj(message_pe_in).view(bs, -1, self.nhead, self.n_dim)
        feat_k = self.prototype_k_proj(feat_pe_in).view(bs, -1, self.nhead, self.n_dim)
        feat_v = self.prototype_v_proj(feat_pe_in).view(bs, -1, self.nhead, self.n_dim)
        prototype = self.prototype_extractor_attention(message_q, feat_k, feat_v, None, feat_mask)
        prototype = prototype.view(bs, -1, self.nhead * self.n_dim)
